...
#define CREATOR_ON_OFF_3 0xB008
#define NVM3KEY_ON_OFF_3 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB008 )
// Creator for attribute: current level, endpoint: 3
#define CREATOR_CURRENT_LEVEL_3 0xB00A
#define NVM3KEY_CURRENT_LEVEL_3 ( NVM3KEY_DOMAIN_ZIGBEE | 0xB00A )
...
DEFINE_BASIC_TOKEN(CURRENT_LEVEL_3, tokType_current_level, 0x00)
#endif // DEFINETOKENS
...
#define GENERATED_TOKEN_LOADER(endpoint) do {\
  ...
  if((endpoint) == 3 || ((endpoint) == EMBER_BROADCAST_ENDPOINT && epNetwork == curNetwork)) { \
    ...
    halCommonGetToken((tokType_current_level *)ptr, TOKEN_CURRENT_LEVEL_3); \
    emberAfWriteServerAttribute(3, ZCL_LEVEL_CONTROL_CLUSTER_ID, ZCL_CURRENT_LEVEL_ATTRIBUTE_ID, (uint8_t*)ptr, ZCL_INT8U_ATTRIBUTE_TYPE); \
  } \
} while(false)
...
#define GENERATED_TOKEN_SAVER do {\
  ...
  if ( endpoint == 1 ) { \
    ...
    if ( clusterId == 0x06 ) { \
      ...
      if ( metadata->attributeId == 0x0000 && 0x0000 == emberAfGetMfgCode(metadata) &&!emberAfAttributeIsClient(metadata) ) \
        halCommonSetToken(TOKEN_CURRENT_LEVEL_3, data); \
    } \
  } \
} while(false)
